...
`, day, year)
	...
	err := os.WriteFile(fmt.Sprintf("aoc%d/day%d.go", year, day), []byte(goTemplate), 0644)
	if err != nil {
		log.Fatalln("failed to create go file", err)
	}

	exTxtTemplate := fmt.Sprint(`// example text here`)
	err = os.WriteFile(fmt.Sprintf("aoc%d/day%d_ex.txt", year, day), []byte(exTxtTemplate), 0644)
	if err != nil {
		log.Fatalln("failed to create example text file", err)
	}

	txtTemplate := fmt.Sprint(`// input text here`)
	err = os.WriteFile(fmt.Sprintf("aoc%d/day%d.txt", year, day), []byte(txtTemplate), 0644)
	if err != nil {
